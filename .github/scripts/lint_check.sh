...
if [ -z "${files}" ]; then
  if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    if [ -n "${GITHUB_BASE_REF:-}" ]; then
     ...
     # Validate base_ref against an allowlist of expected branch names
      case "${base_ref}" in
        master|main)
          ...
          ;;
        *)
          echo "Warning: GITHUB_BASE_REF '${base_ref}' is not in the allowlist; skipping fetch/diff against base ref." >&2
          ;;
      esac
