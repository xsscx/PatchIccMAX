...
print_elapsed_time() {
    ...
    echo "Elapsed Time: $(date -ud "@$ELAPSED_TIME" +'%H:%M:%S')" | tee -a "$LOGFILE"
}
...
run_and_log make -j$(nproc) 2>&1 | tee build_output.log | grep -i 'silent_mode'
...
run_and_log sh run_all_tests.sh > profile-checks.log
run_and_log tail -n 50 profile-checks.log
print_banner "Logs re located in Testing/"
pwd
ls -la *.log
